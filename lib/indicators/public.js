...
export function derivePublicIndicators(input = {}) {
  return {
    ...
    gridFlexibility: input.gridFlexibilityComparable === true
      ? gridFlexibility(input.surplusMw, input.demandMw)
      : unavailable("grid_flexibility"),
...
export function derivePublicIndicatorsFromMetrics(metrics = {}, context = {}) {
  ...
  const input = {
    ...
    // forecast_surplus is Elexon OCNMFD (2–14 day-ahead peak surplus), while
    // transmission_system_demand is an operational outturn. They must not be
    // divided as though they describe the same interval.
    gridFlexibilityComparable: false,
  };
...
export function computeWindow(input={}) {
  const grid = input.gridFlexibilityComparable === true
    ? gridFlexibility(input.surplusMw,input.demandMw)
    : unavailable("grid_flexibility");
  const components=[energyCondition(input.marketPricePerMwh),grid,miningEconomics(input.grossMarginPct),lowCarbonCompute(input.carbonIntensity)].filter((item)=>item.available);
