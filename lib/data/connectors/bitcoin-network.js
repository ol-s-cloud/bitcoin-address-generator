...
export async function getBitcoinNetworkState(options = {}) {
  ...
  try {
    const [statsResult, feesResult, mempoolResult, blocksResult] = await Promise.all([
    ...
    const priceUsd = numberOrNull(stats.market_price_usd);
    ...
    return {
      source: {
        ...
        endpoint: `${BLOCKCHAIN_API}/stats + ${MEMPOOL_API}`,
      ...
      health: {
        ...
        latencyMs: Math.max(statsResult.latencyMs, feesResult.latencyMs, mempoolResult.latencyMs, blocksResult.latencyMs),
