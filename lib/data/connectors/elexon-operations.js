    ...
    normalize(data) {
      ...
      return {
        metric: {
          ...
          horizon: Date.parse(latest.observedAt) > now.getTime() ? "forward" : "current_or_historical",
          forecastLeadHours: Math.max(0, (Date.parse(latest.observedAt) - now.getTime()) / (60 * 60 * 1000)),
        },
  ...
  try {
    ...
    return {
      ...
      health: {
        ...
        // Future timestamps are valid for forecast datasets such as MELNGC.
        // Staleness only applies when the observation timestamp is in the past.
        stale: Date.parse(normalized.latestDataTimestamp) <= Date.now()
          && Date.now() - Date.parse(normalized.latestDataTimestamp) > 6 * 60 * 60 * 1000,
        lastError: null,
      },
    };
  } catch (error) {
