  ...
  try {
    ...
    return {
      ...
      health: {
        ...
        // This dataset is published on a slower cadence than operational telemetry.
        // Keep the connector reachable/operational, but surface observation age
        // explicitly and mark records older than 72h stale for COBRA consumers.
        stale: Date.now() - Date.parse(observedAt) > 72 * 60 * 60 * 1000,
        observationAgeHours: Math.max(0, (Date.now() - Date.parse(observedAt)) / (60 * 60 * 1000)),
        lastError: null,
      },
    };
  } catch (error) {
