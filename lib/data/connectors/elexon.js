...
export async function getElexonMarketPrice(options = {}) {
  ...
  return runConnector({
    ...
    normalize(data) {
      ...
      // MID may legitimately publish zero-volume/defaulted rows when there is
      // insufficient qualifying liquidity or data has not been supplied. A
      // zero-volume row is not a £0/MWh market observation. Walk backwards
      // through the returned settlement periods and use the newest period with
      // positive qualifying volume.
      const times = [...new Set(validRows.map((row) => row.observedAt))].sort(
        (a, b) => Date.parse(b) - Date.parse(a),
      );
      let selectedRows = null;
      let weighted = null;
      for (const time of times) {
        const rowsAtTime = validRows.filter((row) => row.observedAt === time);
        const candidate = volumeWeightedPrice(rowsAtTime);
        if (candidate !== null) {
          selectedRows = rowsAtTime;
          weighted = candidate;
          break;
        }
      }

      if (!selectedRows) throw new Error("elexon_mid_no_positive_volume");

      const selectedTime = selectedRows[0].observedAt;
      return {
        metric: {
          region: "GB",
          ...
          value: weighted,
          unit: "GBP/MWh",
          observedAt: selectedTime,
          settlementDate: selectedRows[0]?.settlementDate ?? null,
          settlementPeriod: selectedRows[0]?.settlementPeriod ?? null,
          providers: selectedRows.map((row) => ({
            id: row.dataProvider,
            price: row.price,
            volume: row.volume,
          })),
          latestReportedAt: validRows.at(-1).observedAt,
        },
        latestDataTimestamp: selectedTime,
      };
    },
  });
}
