...
export async function getElexonSurplus(options = {}) {
  ...
  const endpoint = `${BASE_URL}/datasets/NDZ?publishDateTimeFrom=${encodeURIComponent(from)}&publishDateTimeTo=${encodeURIComponent(to)}&format=json`;
  return runConnector({
    id: "elexon-ndz",
    dataset: "Demand & Surplus Forecast Data - Surplus",
    endpoint,
    fetcher,
    normalize(data) {
      const rows = rowsFrom(data).map(normalizeSurplusRow).filter(Boolean);
      if (!rows.length) throw new Error("invalid_elexon_ndz_payload");
